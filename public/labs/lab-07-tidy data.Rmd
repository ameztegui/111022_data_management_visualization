...
```{r}
nparc_long <- pivot_longer(n_parcelas, 
                           cols = c(IFN_2, IFN_3), 
                           names_to = "IFN")
...
Use `pivot_longer()` and its options to transform the data frame `species` into a **tidy** format, where each column is a variable and each row an observation.
 Remember that `species` contains the number of trees per hectare per each species, plot, and diameter class:
...
```{r}
nparc_wide <- pivot_wider(nparc_long,
                          names_from = IFN,
                          values_from = n_plots)
...
Using the function `pivot_wider()` and its options, transform back the dataframe you created in exercise 1 into a dataframe similar to the original `species`
 data frame.
