...
All of them have a similar structure: the first argument in the function is the `data frame` to which it will be applied, and the rest of arguments specify what to do with this `data frame`, depending on the `verb`we are using. This is a common feature of the `tidyverse`, and also one of its strengths, because many functions share the same structure.

Also, it is important to note that the output of these functions will always be a data frame (as is the input), and that they do not modify the original data frame. That is, if we want to store the result of our process, we need to assign the operations to a given R object, that can be the original data frame (in this case, it will overwrite it) or a new one.
