...
If you are using your own computer, please ensure you have the latest version of R and RStudio installed on your machine. This is important, as some packages used may not install correctly (or at all) if R is not up to date. Remember that R is the name of the programming language itself and RStudio is an IDE (i.e. an interface), so you won't be able to use RStudio without previously installing R.</br></br>
...
Please note that the University machines already have suitable versions of R and RStudio installed.</br></br>



# Introduction to RStudio

Throughout this lesson, we’re going to teach you some of the fundamentals of the R language as well as some best practices for organizing code that will make your life easier. We’ll be using RStudio: a free, open source R integrated development environment. It provides a built-in editor, works on all platforms (including on servers) and provides many advantages such as integration with version control and project management (see the slides of Lesson 1 to know more about the advantages of R and RStudio).
...
The interesting thing is that our variable `x` can now be used in place of a number in any calculation that expects a number:
...
Assignment values can contain the variable being assigned to, and in this case they will overwrite it.

```{r}

x <- x *2 # notice how RStudio updates its description of x on the top right tab

```

> **Warning:** </br>
Although the possibility of overwriting a variable by assigning it a new value is one of the most useful features in R, it comes with  danger. When performing complex tasks, It is **extremely easy** to overwrite a variable inadvertently, and all our calculations from that moment onwards run the risk of being erroneous. 
For example, if you run the copde above twice insted of once, `x` will get a value of `400` instead of `200`. **Always** check the current value of a variable (in the "Environment" panel) before overwriting it.

> **Tip: assignment** </br>
It is also possible to use the `=` operator for assignment, as in: `x = 1/40`. This is much less common among R users, and we will see later why it is not a good idea to use it. If you do use it already, try to change your habits as soon as possible.


# Objects in R

Anytime we assign a variable and store it in R's memory, we are actually creating an `object`. If you look for the `Environment` tab in one of the panes of RStudio, you will see that `x` and its value have appeared. We have previously mentioned that R is an object-oriented language. This basically means that it is designed to use objects as the basis of all the tasks. 
