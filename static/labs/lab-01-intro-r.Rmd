...
Anytime we assign a variable we are actually creating an `object`. If you look for the `Environment` tab in one of the panes of RStudio, you will see that `x` and its value have appeared. We have previously mentioned that R is an object-oriented language. But, what does this mean? It basically means that to perform any type of task we use objects. Everything in R is an object (functions, variables, results …). Thus, entities that are created and manipulated in R are called objects, including data, functions and other structures.
