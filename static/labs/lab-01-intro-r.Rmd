...
## Vectors
One of the most common objects in R is the vector. A vector can store several values, but they **MUST** necessarily be of the **same type** (all numbers, all text, and so forth). There are several ways to create vectors. The most common approach is use the function `c()` which allow us to introduce values manually by separatting them with `,`.
...
Another option to create vectors, but only for vectors contaning numeric values, is the use of `:`, which produces an ordered sequence of numbers by adding 1 starting from the first value and finishing in the last.
