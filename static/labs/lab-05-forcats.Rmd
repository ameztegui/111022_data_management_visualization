...
In R, factors are  a special case of categorical variables, in which variables have a fixed and known set of possible values. Historically, factors were much easier to work with than characters. As a result, many of the functions in base R automatically convert characters to factors. 
...
Sometimes you’d prefer that the order of the levels match the order of the first appearance in the data. You can do that when creating the factor by setting levels to `unique(x)`, or after the fact, with `fct_inorder()`, from the package `forcats`:
...
trees <- trees %>% 
    mutate(Province = fct_recode(Provincia,
                                 "Barcelona"  = "08",
                                 "Girona" = "17",
                                 "Lleida" = "25",
                                 "Tarragona" = "43"))
...
trees <- trees %>% 
    mutate(Province2 = fct_recode(Provincia,
                                 "Barcelona" = "08",
                                 "Lleida" = "25",
                                 "Other" = "17",
                                 "Other" = "43"))
