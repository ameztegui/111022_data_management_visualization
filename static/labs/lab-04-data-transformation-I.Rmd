...
# An introduction to data management with *dplyr*

In this lab we are going to cover the very basics of **data manipulation and transformation**. We will leverage the fundamental functions for data manipulation from the package `dplyr` to start handling our datasets towards the desired analysis and/or visualization procedures.

Before we start, bear in mind that:

- `dplyr` (nor `tidyr`  or `purrr`) does not do anything that can't be done with base R code, `apply` family functions, `for` loops or other packages.  
...
For the development of this lab we are going to use several data objects from the `tibble` class:
...
So, why `tibbles`?. Well, because `tibbles` are not usual `data.frames`, though they are a very similar data structures (class `tbl_df`) they facilitate data inspection and, thus, understanding by:
...
Nonetheless, we can just invoke the objects to check them:
...
## ![](images/04-data-transformation/dplyr.png){width=20px} dplyr
...
![](images/04-data-transformation/data-transformation-cheatsheet-thumbs.png)
...
![](images/04-data-transformation/filter.png){width=250px;align=center}

> The key aspect of `filter` lies on building the *condition* that must be met. Doing so involves the use of `logical operators`:

![](images/04-data-transformation/logical_operators.png){width=250px}
...
2. Get trees in province 43 (Tarragona):

```{r, filter_ex2}
filter(trees, Provincia == '43')
```

3. Get trees in diametric classes 45 and 45:
...
![](images/04-data-transformation/select.png){width=250px}
...
![](images/04-data-transformation/arrange.png)
...
![](images/04-data-transformation/mutate.png)
...
Similar to `select` we can bring in any special function to `mutate`. One useful function is the conditional operator 
`if_else`. It allows to implement a conditional structure in which we determine the output value when a condition is met and the value when it happens otherwise:
